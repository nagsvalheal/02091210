...
import GET_USER_ACCOUNT_ID from "@salesforce/apex/BI_PSPB_EnrollmentUtilities.getEnrolleeCaregiver";
...
export default class BipspbCgNotificationSetting extends LightningElement {
  ...
  wiredAccId({ data, error }) {
    try {
      ...
      if (
        data[0].BI_PSPB_Care_Program_Enrollee__r.BI_PSPB_PatientStatus__c ===
        "Chronic"
      ) {
        this.branded = true;
      }
  ...
  wiredUserCaregiver({ error, data }) {
    ...
    try {
      if (data && data.length > 0) {
        ...
        if (this.phoneRecord === undefined ||this.phoneRecord === null ) {
          console.log('ll')
  ...
  openNotificationSettings(){
    let globalThis = window;
    globalThis.location.assign(this.urlq + resources.CAREGIVER_PROFILE_URL);
